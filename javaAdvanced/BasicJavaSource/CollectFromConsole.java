...
public class CollectFromConsole {
    ...
    public static void main(String[] args) {
        ...
        // int [] :
        int[] intArray = Arrays.stream(scan.nextLine().split("\\s+"))
        ...
        // double[]:
        double[] doubleArray = Arrays.stream(scan.nextLine().split(" "))
        ...
        // List Double:
        List<Double> arrayDouble = Arrays.stream(scan.nextLine().split("\\s+"))
        ...
        List<Character> charList = IntStream.range(0, text.length())
                .mapToObj(text::charAt).collect(Collectors.toList());

        // Queue: String:
        ArrayDeque<String> queueName = Arrays.stream(scan.nextLine()
                .split("\\s+")).collect(Collectors.toCollection(ArrayDeque::new));

        // Integer queue:
        ArrayDeque<Integer> deque = Arrays.stream(scan.nextLine()
        ...
        Map<String, Integer> mapInfo = Arrays.stream(scan.nextLine()
                .split("\\s+")).collect(Collectors.toMap(s -> s, s -> 0, (a, b) -> a, LinkedHashMap::new));
